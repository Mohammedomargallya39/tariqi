...
class SearchDriverController extends GetxController {
  ...
  Future<void> checkForEligibleDriver() async {
    try {
      const String availableDriversUrl = "http://localhost:5000/api/drivers/available";
  ...
  Future<void> _sendUserAcceptance() async {
    try {
      const String acceptEndpoint = "http://localhost:5000/api/rides/accept";
  ...
  Future<void> _checkDriverConfirmation() async {
    ...
    try {
      // This is a placeholder endpoint - replace with actual API
      final String confirmationEndpoint = 
          "http://localhost:5000/api/rides/confirmation/${proposedDriver["rideId"]}";
      
      final response = await http.get(Uri.parse(confirmationEndpoint));
